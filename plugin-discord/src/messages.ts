    ...
    try {
      ...
      const callback: HandlerCallback = async (content: Content, files: any[]) => {
        try {
          ...
          // Construct the reply text to match Agent Scarlett's style
          // Reverting to send content.text directly to match the "Navi" example
          const replyText = content.text;

          const messages = await sendMessageInChunks(
            channel,
            replyText, // Use the direct text from the agent
            message.id,
            files
          );
          ...
          for (const m of messages) {
            ...
            const memory: Memory = {
              ...
              content: {
                ...content,
                text: replyText, // Store the direct text from the agent
                actions,
                inReplyTo: messageId,
                url: m.url,
                channelType: type,
